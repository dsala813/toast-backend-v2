...
def normalize_item_sales_dataframe(df: pd.DataFrame, file_name: str, options: dict[str, Any], max_preview_rows: int) -> dict[str, Any]:
    ...
    if "void_flag" not in df.columns:
        df["void_flag"] = False
        assumptions.append("Added default void_flag=False because no void indicator was present.")

    missing_item_name = int(df["item_name"].isna().sum()) if "item_name" in df.columns else len(df)
    if missing_item_name:
        warnings.append(f"{missing_item_name} rows have missing item_name.")

    duplicate_rows = int(df.duplicated().sum())
    if duplicate_rows:
        warnings.append(f"{duplicate_rows} exact duplicate rows detected in normalized data.")
    ...
    return {
        ...
        "lineage": {
            ...
            "transform_version": "2026-03-31.2",
        },
...
def package_knowledge_from_dataframe(df: pd.DataFrame, file_name: str, options: dict[str, Any], max_preview_rows: int) -> dict[str, Any]:
    ...
    summary_lines = [
        f"# Knowledge Package Summary for {file_name}",
        "",
        f"- Row count: {len(df)}",
    ...
    summary_lines.extend([f"- {c}" for c in df.columns])

    record_preview = df.head(max_preview_rows).fillna("").to_dict(orient="records")
    manifest = {
        "source_type": "toast_export",
        "input_file_name": file_name,
        "row_count": int(len(df)),
    ...
    return {
        "warnings": [],
        "assumptions": ["Knowledge packaging is starter-level and should be expanded for production semantics."],
        "lineage": {
            "input_file_name": file_name,
            "transform_version": "2026-03-31.2",
        },
